...
Interested in helping build intake-thredds? Have code from your work that
...
regarding development in intake-thredds for maintainers and contributors.
...
- Any details about your local setup that might be helpful in troubleshooting,
  specifically the Python interpreter version, installed libraries, and intake-thredds
  version.
...
intake-thredds could always use more documentation. What exactly is needed?
...
2. Clone your fork locally using [git](https://git-scm.com/), connect your repository
   ...
   ```bash
   $ git clone git@github.com:YOUR_GITHUB_USERNAME/intake-thredds.git
   $ cd intake-thredds
   $ git remote add upstream git@github.com:intake/intake-thredds.git
   ```
...
3. Install dependencies into a new conda environment::
   ...
   $ conda activate intake-thredds-dev
   ```

4. Make an editable install of intake-thredds by running::

   ```bash
   $ python -m pip install -e .
   ```

5. Install `pre-commit <https://pre-commit.com>`\_ hooks on the intake-thredds repo::
   ...
   Now you have an environment called `intake-thredds-dev` that you can work in.
